...
public class LoginPage extends Base_Class implements LoginPageInterfaceElements {
	...
	public void loginValidation() throws Throwable {
	    ...
	    Explicit_Wait(PageObjectManager.getPageObjectManager().getFileReader().getDataProperty("explicity_wait"),login_Btn);
		elementClick(login_Btn);
		Explicit_Wait(PageObjectManager.getPageObjectManager().getFileReader().getDataProperty("explicity_wait"),email_Field);
		...
		Explicit_Wait(PageObjectManager.getPageObjectManager().getFileReader().getDataProperty("explicity_wait"),acept_Login);
